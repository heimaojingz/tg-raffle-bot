...
async def handle_create_text(update, context, db):
    ...
    if step == Step.TITLE:
        ...
        await update.message.reply_text(
            _step_text(4, 10, '鑱旂郴鏂瑰紡') + '\n馃幆 <b>鏍囬锛?/b>' + (html.escape(title) or '(鏈缃?') + '\n\n璇疯緭鍏ヨ仈绯绘柟寮忥細',
        ...
        try:
            ...
            await update.message.reply_text('鈿狅笍 鏍煎紡閿欒锛岃杈撳叆 YYYY-MM-DD HH:MM锛屽 2026-07-05 20:00', parse_mode='HTML', reply_markup=_kb([flow_back_btn, cancel_btn]))
            return True
        ...
        await update.message.reply_text(
            '濂栧搧銆' + html.escape(user_text) + '銆嶇殑涓浜烘暟锛',
            parse_mode='HTML', reply_markup=_kb([
                ...
                [InlineKeyboardButton('8', callback_data='create_pcount_8'),
                 ...
                 InlineKeyboardButton('鉁忥笍 鑷畾涔', callback_data='create_pcount_custom')],
                cancel_btn
            ])
        )
        return True
        ...
        await update.message.reply_text('鉁?宸叉坊鍔犻閬擄細' + display, parse_mode='HTML')
...
async def _show_channel_prompt(update, context):
    msg = _step_text(5, 10, '棰戦亾璁㈤槄') + '\n璇疯緭鍏ヨ闃呴摼鎺ワ紙璁㈤槄鍚庢墠鍙互鍙備笌锛夛細\n鏍煎紡锛氬悕绉皘閾炬帴\n渚嬪锛氶噾鎮﹀皬濮愬|https://t.me/+xxxxxx'
...
async def _show_prize_input(update, context, db, is_new=True):
    ...
    if prizes:
        ...
        if row:
            keyboard.append(row)
    keyboard.append([InlineKeyboardButton('鉁忥笍 鑷畾涔夎緭鍏', callback_data='create_prize_custom')])
    ...
    if data == 'create_skip_desc':
        ...
        await query.edit_message_text(
            ...
            parse_mode='HTML', reply_markup=_kb([
                ...
                [InlineKeyboardButton('鉁忥笍 鑷畾涔夋椂闂', callback_data='create_time_custom')],
        ...
        await query.edit_message_text(
            ...
            parse_mode='HTML', reply_markup=_kb([
                ...
                [InlineKeyboardButton('500', callback_data='create_dcount_500'),
                 InlineKeyboardButton('鉁忥笍 鑷畾涔', callback_data='create_dcount_custom')],
                cancel_btn
            ])
        )
        ...
        if prize_name:
            ...
            context.user_data['create_step'] = Step.PRIZE_COUNT
            await query.edit_message_text(
                '濂栧搧銆' + html.escape(prize_name) + '銆嶇殑涓浜烘暟锛',
                parse_mode='HTML', reply_markup=_kb([
                    [InlineKeyboardButton('1', callback_data='create_pcount_1'),
                     InlineKeyboardButton('3', callback_data='create_pcount_3'),
                     InlineKeyboardButton('5', callback_data='create_pcount_5')],
        ...
        await query.answer()
        context.user_data['create_step'] = Step.ADD_PRIZE
        await query.edit_message_text(_step_text(7, 10, '濂栧搧') + '\n璇疯緭鍏ュ鍝佸悕绉帮細\n渚嬪锛?00鍏冧紭鎯犲埜鍖呭', parse_mode='HTML', reply_markup=_kb([cancel_btn]))

    elif data.startswith('create_pcount_'):
        await query.answer()
        if data == 'create_pcount_custom':
            context.user_data['create_step'] = Step.PRIZE_COUNT
            await query.edit_message_text('濂栧搧銆' + html.escape(context.user_data.get('_prize_name', '')) + '銆嶇殑涓浜烘暟锛焅n璇疯緭鍏ユ暟瀛楋細', parse_mode='HTML', reply_markup=_kb([cancel_btn]))
        else:
        ...
        if not context.user_data['create_data'].get('prizes'):
            await query.answer('鑷冲皯闇€瑕佷竴涓鍝', show_alert=True)
            return
        ...
        await query.edit_message_text(
            ...
            parse_mode='HTML', reply_markup=_kb([
                ...
                [InlineKeyboardButton('鉁忥笍 鑷畾涔夊叧閿瘝', callback_data='create_kw_custom')],
                cancel_btn
            ])
        )
        ...
        if data == 'create_kw_custom':
            await query.edit_message_text(_step_text(8, 10, '鍏抽敭璇') + '\n璇疯緭鍏ヨЕ鍙戝叧閿瘝锛歕n渚嬪锛氭娊濂', parse_mode='HTML', reply_markup=_kb([cancel_btn]))
        else:
        ...
        await query.answer()
        context.user_data['create_step'] = Step.CHANNEL
        await query.edit_message_text('缁х画娣诲姞棰戦亾锛歕n鏍煎紡锛氬悕绉皘閾炬帴\n渚嬪锛氶噾鎮﹀皬濮愬|https://t.me/+xxxxxx', parse_mode='HTML', reply_markup=_kb([skip_btn('create_skip_channel'), cancel_btn]))
        ...
        step = context.user_data.get('create_step')
        data_dict = context.user_data.get('create_data', {})
        contactedisp = '\n\n褰撳墠鑱旂郴鏂瑰紡锛' + html.escape(contacted) + '\n\n璇疯緭鍏ユ柊鑱旂郴鏂瑰紡锛堟垨鐩存帴鍙戦€佽鐩栵級锛'
        describedisp = '\n\n褰撳墠璇存槑锛' + html.escape(described) + '\n\n璇疯緭鍏ユ柊璇存槑锛堟垨鐩存帴鍙戦€佽鐩栵級锛'
        titledisp = '\n\n褰撳墠鏍囬锛' + html.escape(titled) + '\n\n璇疯緭鍏ユ柊鏍囬锛堟垨鐩存帴鍙戦€佽鐩栵級锛'
        ...
        if prev:
            ...
            if prev == Step.TITLE:
                ...
                await query.edit_message_text(_step_text(5, 10, '棰戦亾璁㈤槄') + '\n璇疯緭鍏ヨ闃呴摼鎺ワ細\n鏍煎紡锛氬悕绉皘閾炬帴\n渚嬪锛氶噾鎮﹀皬濮愬|https://t.me/+xxxxxx', parse_mode='HTML', reply_markup=_kb([skip_btn('create_skip_link'), flow_back_btn, cancel_btn]))
                ...
                await query.edit_message_text(_step_text(5, 10, '棰戦亾璁㈤槄') + '\n褰撳墠棰戦亾锛歕n' + lines + '\n\n鏍煎紡锛氬悕绉皘閾炬帴\n渚嬪锛氶噾鎮﹀皬濮愬|https://t.me/+xxxxxx', parse_mode='HTML', reply_markup=_kb([skip_btn('create_skip_channel'), cancel_btn]), disable_web_page_preview=True)
        else:
        ...
        sub_note = '\n  锛堣闃呭悗鎵嶅彲浠ュ弬涓庢娊濂栵級' if ch_list else ''
...
async def handle_publish_callback(update, context, db):
    ...
    if not chat_id:
        await query.answer('鏃犳晥鐨勯閬撻摼鎺', show_alert=True)
        return
    ...
    if chat_id.startswith('@'):
        try:
            ...
            if bot_member.status not in ['administrator', 'creator', 'member']:
                await query.answer('鏈哄櫒浜轰笉鏄?' + chat_id + ' 鐨勬垚鍛橈紝璇峰厛灏嗘満鍣ㄤ汉鍔犲叆棰戦亾骞惰涓虹鐞嗗憳', show_alert=True)
                return
        except Exception:
            await query.answer('鏃犳硶璁块棶 ' + chat_id + '锛岃纭鏈哄櫒浜哄凡鍦ㄩ閬撲腑涓斾负绠＄悊鍛', show_alert=True)
            return
...
async def _show_confirm(update, context, db):
    ...
    try:
        ...
        if not data:
            if update.callback_query:
                await update.callback_query.answer('\u26a0\ufe0f 鏁版嵁涓㈠け锛岃閲嶆柊鍒涘缓娲诲姩', show_alert=True)
            return
